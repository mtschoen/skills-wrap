...
- ~~**1. Package wrap as an installable Claude Code plugin.**~~ Shipped, with a correction: this item's proposed manifest was wrong. There is no `skills` field, and a plugin discovers skills under `skills/<name>/`, not at the plugin root - verified against the official marketplace's own plugins. Since restructuring the repo would break the skills-dev root layout, `scripts/build-plugin.sh` generates the plugin tree instead, and `.claude-plugin/plugin.json` + `hooks/hooks.json` are committed. **Remaining decision:** `/plugin install <git-url>` needs the generated `skills/wrap/` tree committed to the repo. Today the tree is build-time only, so plugin install works from a local path but not straight from the GitHub URL.
