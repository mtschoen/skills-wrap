...
3. **Named teammate agents** (spawned via the Agent tool with a `name:`, addressable through `SendMessage`). These do NOT exit when their task ends - they linger idle (emitting `idle_notification` heartbeats) and survive past the wrap unless explicitly shut down. Sweep them like tasks: first confirm each one's final report was harvested (an agent that went idle WITHOUT reporting owes one - ping it via `SendMessage` and absorb the reply), then terminate each with a `SendMessage` shutdown request: `{"type": "shutdown_request", "reason": "session wrap complete"}` - the agent approves and its process terminates. Skipping this leaves a fleet of idle agents outliving the closed session (observed 2026-07-01: nine idle teammates lingering hours past their work).
