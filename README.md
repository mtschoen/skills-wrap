...
2. Register a session-end hook in the agent's settings (Claude Code: the `SessionEnd` hook in `~/.claude/settings.json`) — use the `update-config` skill, pointing it at `hooks/session-end-reminder.sh` (Unix/macOS) or `hooks/session-end-reminder.ps1` (Windows). This step depends on the harness supporting session-end hooks; not every harness does.
3. (Optional) The `scripts/find-unwrapped.sh` / `.ps1` companions are diagnostics that scan the agent's session transcripts (Claude Code: `~/.claude/projects`) and are not required for `/wrap` itself - copy `scripts/` too if you want them, or just run them from a checkout of this repo. Override the default transcript root with the `AGENT_SESSIONS_DIR` env var.
